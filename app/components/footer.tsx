"use client";

import React from "react";
import Link from "next/link";
import { usePathname } from "next/navigation";

const Footer: React.FC = () => {
  const pathname = usePathname();

  const footerLinks = [
    { name: "Home", href: "/" },
    { name: "Experts", href: "/experts" },
    { name: "About us", href: "/about" },
    { name: "Podcasts", href: "/podcast" },
    { name: "Contacts", href: "/contact" },
  ];

  return (
    <footer className="w-full px-6 sm:px-10 lg:px-20 pt-24 lg:pt-40 pb-10 text-white">
      {/* Footer Navigation */}
      <nav className="flex flex-wrap justify-center gap-6 sm:gap-10 text-base sm:text-lg tracking-wide mb-10 text-center">
        {footerLinks.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className={`transition-opacity hover:opacity-70 ${
              pathname === link.href
                ? "opacity-100 underline underline-offset-8"
                : "opacity-70"
            }`}
          >
            {link.name}
          </Link>
        ))}
      </nav>

      {/* Bottom Content */}
      <div className="w-full max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
        {/* Left */}
        <div className="flex items-center gap-2 text-sm sm:text-lg tracking-wide">
          <svg width="20" height="16" viewBox="0 0 20 16" fill="none">
            <path
              d="M4.81132 16C4.90566 16 5 15.966 5.09434 15.8981L9.43396 12.2972C9.55975 12.2972 9.65409 12.2972 9.74843 12.2972C9.81132 12.2972 9.90566 12.2972 10 12.2972C15.5975 12.2972 20 9.57962 20 6.14862C20 2.68365 15.5975 0 10 0C4.40252 0 0 2.71762 0 6.14862C0 6.69214 0.0943394 7.23567 0.283018 7.74522C0.377357 8.01699 0.660378 8.1189 0.911949 8.01699C1.16352 7.91507 1.25786 7.60934 1.16352 7.33758C1.00629 6.92994 0.943396 6.52229 0.943396 6.11465C0.943396 3.32909 5.09434 0.985138 10 0.985138C14.9057 0.985138 19.0566 3.32909 19.0566 6.11465C19.0566 8.90021 14.9057 11.2442 10 11.2442C9.90566 11.2442 9.81132 11.2442 9.71698 11.2442C9.59119 11.2442 9.46541 11.2442 9.27673 11.2442C9.15094 11.2442 9.0566 11.2781 8.96226 11.3461L5.37736 14.3355L5.75472 11.1762C5.78616 10.9384 5.62893 10.7006 5.40881 10.6327C4.71698 10.4289 4.0566 10.1571 3.49056 9.81741C3.27044 9.68153 2.98742 9.78344 2.86163 10.0212C2.73585 10.259 2.83019 10.5648 3.05032 10.7006C3.55346 11.0064 4.1195 11.2442 4.74843 11.448L4.33962 15.4225C4.30818 15.6263 4.40252 15.8302 4.55975 15.9321C4.65409 16 4.74843 16 4.81132 16Z"
              fill="white"
            />
          </svg>
          <span>Let’s chat</span>
        </div>

        {/* Right */}
        <div className="flex items-center gap-2 text-sm sm:text-lg tracking-wide">
          <svg
            width="20"
            height="14"
            viewBox="0 0 20 14"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            {" "}
            <path
              d="M19.3818 10.2931C19.6727 10.2931 19.9273 10.0411 19.9273 9.75321V0.503856C19.9273 0.503856 19.9273 0.503856 19.9273 0.467866C19.9273 0.431876 19.9273 0.431877 19.9273 0.395887V0.323907C19.9273 0.323907 19.9273 0.287917 19.8909 0.287917C19.8909 0.287917 19.8909 0.251928 19.8545 0.251928L19.8182 0.215938C19.8182 0.215938 19.8182 0.215938 19.8182 0.179949L19.7091 0.071979C19.7091 0.071979 19.6727 0.0719796 19.6727 0.0359899C19.6727 0.0359899 19.6364 0.0359897 19.6364 0H19.6C19.6 0 19.5636 0 19.5273 0H19.4909C19.4909 0 19.4909 0 19.4545 0H0.472727C0.472727 0 0.472727 0 0.436363 0C0.4 0 0.4 0 0.363636 0H0.327273C0.290909 0 0.290909 0 0.290909 0C0.290909 0 0.254545 1.3586e-07 0.254545 0.0359899C0.254545 0.0359899 0.218181 0.0359893 0.218181 0.071979C0.218181 0.071979 0.181818 0.0719792 0.181818 0.107969L0.109091 0.179949C0.109091 0.179949 0.109091 0.179949 0.109091 0.215938L0.0727269 0.251928C0.0727269 0.251928 0.0727267 0.287917 0.0363631 0.287917C0.0363631 0.287917 0.0363636 0.323907 0 0.323907V0.395887C0 0.431877 0 0.431876 0 0.467866C0 0.467866 0 0.467866 0 0.503856V13.4602C0 13.7481 0.254545 14 0.545455 14H19.4545C19.7455 14 20 13.7481 20 13.4602C20 13.1722 19.7455 12.9203 19.4545 12.9203H1.01818V1.65553L9.56364 8.63753C9.6 8.67352 9.67273 8.70951 9.70909 8.70951H9.74545C9.78182 8.70951 9.85455 8.7455 9.89091 8.7455C9.92727 8.7455 10 8.7455 10.0364 8.70951H10.0727C10.1091 8.67352 10.1818 8.67352 10.2182 8.63753L18.7636 1.65553V9.75321C18.8364 10.0411 19.0909 10.2931 19.3818 10.2931ZM9.92727 7.52185L2 1.0437H17.8545L9.92727 7.52185Z"
              fill="white"
            />{" "}
          </svg>
          <a
            href="mailto:info@logoipsum.com"
            className="hover:opacity-70 transition"
          >
            info@logoipsum.com
          </a>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
