...
const PodcastOfWeek: React.FC = () => {
  return (
    <section
      className="relative min-h-screen w-full bg-cover bg-center text-white px-14 py-16"
    >
      {/* Header */}
      <div className="flex flex-col lg:flex-row justify-evenly gap-12 mb-20">
        <h1 className="text-4xl md:text-[5rem] font-extrabold tracking-widest uppercase">
          Podcast of the Week
        </h1>

        <div className="max-w-2xl text-xl text-gray-200">
          ...
          <button className="flex items-center gap-2 border-white border-3 px-6 py-1 rounded-md text-lg font-semibold hover:bg-white hover:text-black transition">
            Learn More
      ...
      <div className="grid grid-cols-2 gap-x-32 p-20">
        {/* LEFT COLUMN */}
        <div className="flex flex-col">
          {leftColumn.map((ep, index) => (
            <React.Fragment key={ep.number}>
              <div className="flex items-center gap-8">
                <span className="text-3xl font-extrabold">{ep.number}</span>
                ...
                <div>
                  ...
                  <p className="text-md font-bold tracking-widest">{ep.tags}</p>
                </div>
              </div>

              {index === 0 && <div className="h-0.5 bg-white w-full my-16" />}
            </React.Fragment>
          ))}
        </div>
